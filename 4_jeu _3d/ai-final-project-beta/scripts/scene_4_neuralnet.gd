...
# 32 chemins au total : 12 (I→H1) + 16 (H1→H2) + 4 (H2→O)
...
# combinaison correcte : aucun croisement, W=1/x maximal sur chaque segment
...
# positions X world de chaque neurone — pour la détection géométrique de croisements
...
var _path_nodes: Dictionary = {}   # name → CSGBox3D (mesh du chemin dans $Paths)
var _path_active: Dictionary = {}  # name → bool (activé par le joueur)
var _path_bodies: Dictionary = {}  # name → StaticBody3D (collision marchable)
var _path_kills: Dictionary = {}   # name → Area3D (zone de mort si chemin rouge)
var _btn_meshes: Dictionary = {}   # name → CSGBox3D (mesh du bouton pour colorisation)
...
func _ready() -> void:
	...
	_build_materials()  # crée les matériaux partagés (vert / rouge / off / indicateur)
	_init_paths()       # collecte les CSGBox3D de $Paths et les masque tous
	_init_buttons()     # configure les boutons du panneau (taille, layer, signaux)
	_init_minimap()     # branche la texture SubViewport sur l'écran minimap
	_sophia.left_click_pressed.connect(_on_raycast_interact)  # clic gauche → toggle à distance
	$KillZone.body_entered.connect(_on_kill_zone_body_entered)
	# la clé est cachée jusqu'à ce que la solution exacte soit activée
	$KeyPickup.visible = false
	$KeyPickup.monitoring = false
	_hud.set_subtitle("")
	_constraints_label = $ConstraintsRulesHUD/VBox/ConstraintsLabel
	_update_constraints_display([])  # affiche les règles initiales (0 chemins actifs)


func _build_materials() -> void:
	# chemin valide — vert semi-transparent marchable
	...
	# chemin croisé / invalide — rouge semi-transparent avec kill zone
	...
	# bandes de bord cyan sur les chemins verts (indicateur "marchable")
...
func _init_paths() -> void:
	# tous les chemins commencent invisibles — activés un par un par le joueur
	for path_node in $Paths.find_children("*", "CSGBox3D"):
		...
		path_node.visible = false  # invisible = pas de collision, pas de kill zone
...
func _init_buttons() -> void:
	for path_name in PATH_NAMES:
		...
		# redimensionne le bouton pour qu'il soit plus visible sur le panneau
		(_btn_meshes[path_name] as CSGBox3D).size.y = 0.6
		(_btn_meshes[path_name] as CSGBox3D).position.y = 0.7
		zone.position.y = 0.7
		zone.collision_layer = 4  # layer 4 = zones de boutons (raycast mask=4)
...
func _init_minimap() -> void:
	# branche la texture live du SubViewport sur le matériau de l'écran MapScreen
	...
	mat.emission_texture = vp.get_texture()  # même texture en émissif pour que l'écran brille
...
func _on_btn_interact(path_name: String) -> void:
	# toggle l'état actif du chemin puis reconstruit tout le réseau
...
func _update_all_paths() -> void:
	# reconstruit les croisements à chaque toggle (calcul global, pas incrémental)
	...
	# applique l'état (off / green / red) à chaque chemin
	for path_name in PATH_NAMES:
	...
	# met à jour la couleur de chaque bouton en miroir du chemin
	...
	# vérifie si la solution exacte est atteinte pour révéler la clé
...
func _update_constraints_display(active_list: Array) -> void:
	...
	# vérifie que chaque plateforme source a exactement 2 chemins sortants
...
func _set_path_state(path_name: String, state: String) -> void:
	...
	if state == "off":
		...
		return
	# green ou red : visible + crée un StaticBody3D marchable si absent
	...
	if state == "green":
		...
		if not _path_kills.has(path_name):
			...
			kb.size = Vector3(path_node.size.x, 0.5, path_node.size.z)  # légèrement plus épais que la trace
			kc.shape = kb
			kill.add_child(kc)
...
func _create_indicators(path_name: String, path_node: CSGBox3D) -> void:
	if _path_indicators.has(path_name):
		return  # déjà créés pour ce chemin
	var sz: Vector3 = path_node.size
	var indicators: Array = []
	# deux bandes cyan, une de chaque côté du chemin (Z+ et Z-)
	for side in [-1, 1]:
...
# deux chemins se croisent si leurs extrémités X s'inversent (critère géométrique)
# uniquement valable dans le même layer (L1 ou L2) ; L3 converge vers O → jamais croisé
func _paths_cross(a: String, b: String) -> bool:
	var la := _get_layer(a)
	var lb := _get_layer(b)
	if la != lb or la == 3:  # L3 : tous convergent vers O, pas de croisement possible
		return false
	...
	return (xa - xb) * (ya - yb) < 0.0  # signe négatif = inversion des X = croisement
...
func _on_raycast_interact() -> void:
	# clic gauche → raycast 15 unités sur layer 4 pour toggler un bouton à distance
	...
	params.collision_mask = 4  # layer 4 = zones de boutons uniquement
